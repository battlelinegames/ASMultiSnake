...
export function initASWebGLue(importObject) {
  ...
  importObject.webgl.getArrayView = (arr_ptr) => {
    ...
    const align = 31 - Math.clz32((info >>> VAL_ALIGN_OFFSET) & 31); // -1 if none getValueAlign(info)
    ...
    return webgl.getView(align, info & VAL_SIGNED, info & VAL_FLOAT)
  ...
  const bufferdata = (ctx, target, data, usage) => {
    // try {
    //console.log("calling bufferdata")
